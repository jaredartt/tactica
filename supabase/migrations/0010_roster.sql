...
-- ===========================================================================
--  0010 — the real roster, and four things units can now do
--
--  The six placeholders retire. In their place, eight characters with their
--  own art, their own name, and a class shown beside it. The slug is the
--  name: it is what a saved deck stores and what a unit carries into a match,
--  so it is the one thing that must not change once people own decks.
--
--  Four new abilities, and each is a flag on the card rather than a rule with
--  a name in it -- so the next flying thing you draw is a column update, not
--  a code change:
--
--    flies      movement ignores trees AND bodies; it still cannot end its
--               move on top of either. Lumea.
--    sneaks     never takes a counter. Mako -- a thief that trades blows is
--               not a thief.
--    cures      mending also puts out a burn. Umiro, so a Mage is not simply
--               a permanent tax once it has landed one hit.
--    tramples   walking into a tree destroys it and the walk stops there.
--               Wuzu. It turns terrain from cover into something you can
--               spend a turn removing.
--
--  Art lives in the repository and is served by GitHub Pages, not from
--  Storage: it is static, it costs no egress against the database, and if
--  someone hammers it, the thing that slows down is a static file host rather
--  than the server holding every match. art_url is therefore a path relative
--  to wherever the app is deployed, and the client prefixes it.
-- ===========================================================================

alter table public.cards add column if not exists role     text not null default '';
alter table public.cards add column if not exists flies    boolean not null default false;
alter table public.cards add column if not exists sneaks   boolean not null default false;
alter table public.cards add column if not exists cures    boolean not null default false;
alter table public.cards add column if not exists tramples boolean not null default false;

-- Everything that came before leaves the roster. Retired, not deleted: a
...
insert into public.cards
  (slug, name, role, hp, mov, rmin, rmax, crmin, crmax, dmin, dmax,
   burns, heals, flies, sneaks, cures, tramples,
   attack, move, range, ability, accent, art_url, sort, is_active)
values
  ('dione-grifo', 'Dione & Grifo', 'Swordsmen', 110, 2, 1, 1, 1, 2, 18, 26,
   false, false, false, false, false, false, 22, 2, 1,
   'Back to back. Answers a blow from one tile away or from two.',
   '#e03050', 'cards/dione-grifo.webp', 1, true),

  ('wuzu', 'Wuzu', 'Monster', 120, 1, 1, 1, 1, 1, 30, 42,
   false, false, false, false, false, true, 36, 1, 1,
   'Walks through the woods. Trees it steps into come down.',
   '#8a6b3a', 'cards/wuzu.webp', 2, true),

  ('mako', 'Mako', 'Bandit', 60, 3, 1, 1, 1, 1, 20, 30,
   false, false, false, true, false, false, 25, 3, 1,
   'Gone before you turn round. Never takes a blow in return.',
   '#d2601a', 'cards/mako.webp', 3, true),

  ('lumea', 'Lumea', 'Flyer', 70, 3, 1, 1, 1, 1, 15, 22,
   false, false, true, false, false, false, 18, 3, 1,
   'Goes over the trees and over your line. Nothing on the ground is in the way.',
   '#6f6fd8', 'cards/lumea.webp', 4, true),

  ('dereo', 'Dereo', 'Mage', 70, 2, 2, 2, 1, 2, 15, 25,
   true, false, false, false, false, false, 20, 2, 2,
   'Sets what it strikes alight. A burning unit loses 5 HP whenever it swings.',
   '#e8503c', 'cards/dereo.webp', 5, true),

  ('fey', 'Fey', 'Mage', 65, 2, 2, 3, 3, 3, 12, 20,
   false, false, false, false, false, false, 16, 2, 3,
   'Reaches two or three tiles, and only something with the same reach answers.',
   '#7c5cc4', 'cards/fey.webp', 6, true),

  ('eva', 'Eva', 'Herbalist', 70, 3, 1, 2, 1, 2, 5, 15,
   false, true, false, false, false, false, 10, 3, 2,
   'Quick on her feet. Mends an ally for as much as she would hurt a foe.',
   '#3f8f4a', 'cards/eva.webp', 7, true),

  ('umiro', 'Umiro', 'Herbalist', 95, 1, 1, 2, 1, 1, 10, 20,
   false, true, false, false, true, false, 15, 1, 2,
   'Slow, and hard to shift. What he mends stops burning.',
   '#b0523f', 'cards/umiro.webp', 8, true)
...
-- ---------------------------------------------------------------------------
-- 1. the new flags travel with the unit onto the board
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. where a unit can walk, now that some of them do not walk
--
-- An overload taking the whole unit rather than a movement number, because
-- three different things about a unit now change the answer. The old
-- four-argument version is left alone; nothing calls it any more.
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_body boolean[]; v_tree boolean[]; v_seen boolean[]; v_front int[]; v_next int[];
    ...
    foreach i in array v_front loop
      ...
      for k in 1 .. 4 loop
        ...
        if nx >= 0 and ny >= 0 and nx < v_w and ny < v_h then
          ...
          if not v_seen[j] and not v_body[j] and (v_tramples or not v_tree[j]) then
            v_seen[j] := true;
            v_next := v_next || j;
            v_out  := v_out || (nx || ',' || ny);
          end if;
...
-- ---------------------------------------------------------------------------
-- 3. moving, and what a trampler leaves behind
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. striking, mending, and the two units that break the usual bargain
-- ---------------------------------------------------------------------------
...
begin
  ...
  v_st := jsonb_set(v_st, '{fx}', jsonb_build_object(
    ...
    'cured', v_cured, 'tree', (v_tree is not null)));
...
-- The bot asks the same question about movement that everyone else does, so
-- it flies when it is flying something and walks through trees when it is
-- not. One line, and it inherits both abilities.
...
revoke execute on function public.cn_reach(jsonb, jsonb) from public, anon, authenticated;

-- ---------------------------------------------------------------------------
-- Did it work? All true means yes.
-- ---------------------------------------------------------------------------
select
  (select count(*) from public.cards where is_active) = 8              as roster_is_eight,
  (select flies    from public.cards where slug = 'lumea')             as lumea_flies,
  (select sneaks   from public.cards where slug = 'mako')              as mako_sneaks,
  (select cures    from public.cards where slug = 'umiro')             as umiro_cures,
  (select tramples from public.cards where slug = 'wuzu')              as wuzu_tramples,
  (select art_url  from public.cards where slug = 'fey') is not null   as art_is_wired;
